...
void ModBusSlave::Device::Port::setPin( int i1, const ModBusSlave::Device::Port::Pin& p_pin )
{
   pin[i1] = p_pin;
}
...
ModBusTcp::Adu::Exception ModBusSlave::readRegister( ModBusTcp::ReadRegister* cmd, bool inputsOnly )
{
...
uint8_t ModBusSlave::readStatus( uint16_t registerId, uint8_t length, uint8_t* data )
{
...
ModBusTcp::Adu::Exception ModBusSlave::readStatus( ModBusTcp::ReadStatus* cmd, bool inputsOnly )
{
   uint16_t addr = cmd->getAddress();
   uint16_t length = cmd->getLength();
...
void ModBusSlave::sendCommand( const ModBusSlave::RegisterAddress& regAddr )
{
   ...
   Device::Port::Pin& pin = device[regAddr.device].port[regAddr.port].pin[regAddr.pin];
   IResponse cmd( getId() );
   ...
   if ( hacfClassId[pin.id] == Object::ClassId::DIGITAL_OUTPUT )
   {
      ...
      psCmd->setCommand( pin.value ? PortPinUnit::Command::ON : PortPinUnit::Command::OFF );
...
ModBusTcp::Adu::Exception ModBusSlave::writeRegister( ModBusTcp::WriteRegister* cmd )
{
   uint16_t value = cmd->getValue();
   RegisterAddress addr( cmd->getAddress() );

   DEBUG_H4( FSTR( "writeRegister to 0x" ), addr.getValue(), FSTR( ", value 0x" ), value );

   if ( addr.getValue() >= ( sizeof( device ) / 2 ) )
   {
      return ModBusTcp::Adu::EX_ILLEGAL_DATA_ADDRESS;
   }
   Device::Port::Pin& pin = device[addr.device].port[addr.port].pin[addr.pin];
   if ( isInput( pin.id ) )
   {
...
ModBusTcp::Adu::Exception ModBusSlave::writeMultipleRegister( ModBusTcp::WriteMultipleRegister* cmd )
{
   uint16_t length = cmd->getLength();
   RegisterAddress addr( cmd->getAddress() );

   DEBUG_H4( FSTR( "writeMultipleRegister to 0x" ), addr.getValue(), FSTR( ", length 0x" ),
             length );

   if ( !length || ( ( addr.getValue() + length ) >= ( sizeof( device ) / 2 ) ) )
   {
      return ModBusTcp::Adu::EX_ILLEGAL_DATA_ADDRESS;
   }

   Device::Port::Pin& pin
      = device[addr.device].port[addr.port].pin[addr.pin];
   if ( isInput( pin.id ) )
   {
      return ModBusTcp::Adu::EX_ILLEGAL_DATA_VALUE;
   }
   ...
   ModBusTcp::WriteMultipleCoils* cmd )
{
   ...
   DEBUG_H4( FSTR( "writeMultipleCoils to 0x" ), addr.getValue(), FSTR( ", length 0x" ), length );
